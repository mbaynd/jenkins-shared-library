...
def sast(String projectName, String credentialsId) {
    ...
    withSonarQubeEnv(insatllationName: 'sonarqube', envOnly: true, credentialsId: "${credentialsId}") {
        sh "${sonarScannerHome}/bin/sonar-scanner -Dsonar.sources=. -Dsonar.projectName=${projectName} -Dsonar.projectKey=${projectName} -Dsonar.analysis.buildNumber=$BUILD_NUMBER -Dsonar.java.binaries=./target/classes"
    }
